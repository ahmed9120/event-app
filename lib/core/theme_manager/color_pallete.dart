import 'dart:ui';

abstract class ColorPallete{
  static const Color primaryColor= Color(0XFF5669FF);
  static const Color lightBackgroundColor= Color(0XFFF2FEFF);
  static const Color darkBackgroundColor= Color(0XFF101127);
  static const Color darkThemeHomeTextColor= Color(0XFFF4EBDC);
  static const Color white= Color(0XFFFFFFFF);
  static const Color textFiledBorderColor= Color(0XFF7B7B7B);
  static const Color generalTextColor= Color(0XFF1C1C1C);
  static const Color textFiledBorderErrorColor= Color(0XFFFF5659);
  static const Color appbarTitleColor= Color(0XFF101127);
  static const Color deleteRedColor= Color(0XFFFF5659);


}